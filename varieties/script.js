...
function openRecipeDetails(recipe) {
  ...
  container.innerHTML = `
    <div class="recipe-details">
      ...
      <h3 class="orh3">Ingredients</h3>
      <ul class="fulldetailul">
        ${recipe.extendedIngredients.map(ing => `<li>${ing.original}</li>`).join('')}
      </ul>
      <h3 class="orh3">Instructions</h3>
      <p class="fulldetailul">${recipe.instructions || "Instructions not available."}</p>
      <button class="back-button" onclick="reloadPage()">&#8592; Close</button>
    </div>
  `;
}
